      ...
      try {
        ...
        if (res.status === 200) {
          ...
          for (let item of this.projects) {
            if (item.communityname !== '' && !this.communities.includes(item.communityname)) {
              this.communities.push(item.communityname);
            }
            if (item.districtname !== '' && !this.districtname.includes(item.districtname)) {
              this.districtname.push(item.districtname);
            }
            if (item.status_id !== '') {
              const status = this.projectStatus.find(status => status.id === item.status_id);
  ...
  computed: {
    currentPageProjects() {
      ...
      if (this.communities_select != 'ชุมชน ...'){
        ...
        return (
              item.communityname.toString().includes(this.communities_select)
            );
        });
      ...
      if (this.districtname_select != 'เขต ...'){
        const filteredData = data_list.filter(item => {
        return (
              item.districtname.toString().includes(this.districtname_select)
            );
        });
        data_list = filteredData;
      }
      ...
      if (this.statusname_select != 'สถานะ ...'){
        ...
        const filteredData = data_list.filter(item => {
        return (
              item.status_id.toString().includes(statusid)
            );
        });
        data_list = filteredData;
      }
...
<template>
  ...
  <NavBar />
  <!-- test -->
  <!-- <p class="c-1" style="color :rgba(1, 243, 102, 0.699)">dev_msg : {{ this.search }}</p> -->
  <div class="container" style="margin-top: 20px">
    <div class="row">
      ...
      <div class="dropdown d-flex align-items-center justify-content-between">
        <div class="pe-2">
          <h6>ชุมชน</h6>
          <select style="width : 300px;" class="form-select" aria-label="Default select example" v-model="communities_select">
        ...
        <div class="pe-2">
          <h6>เขต</h6>
          <select style="width : 200px;" class="form-select" aria-label="Default select example" v-model="districtname_select">
        ...
        <div class="pe-2">
          <h6>สถานะ</h6>
          <select style="width : 200px;" class="form-select" aria-label="Default select example" v-model="statusname_select">
        ...
        <div class="pe-2">
          <h6>ค้นหา</h6>
          <div style="width : 300px;  border : 1px solid #929292" class="btn-group">
            <input type="text" class="form-control" aria-label="Text input with dropdown button" placeholder="ชื่อโครงการ, แขวง, ชื่อชุมชน" v-model="search"> 
          </div>
        </div>
      </div>
          ...
          <tbody>
            <template v-for="item in currentPageProjects">
              <tr class="small font-weight-normal">
                ...
                <td>
                  <button type="button" class="btn btn-outline-primary me-2" style="background: rgb(239, 77, 78); border-radius: 4px; color: rgb(255, 255, 255); border-color: rgb(239, 77, 78); font-size: 14px;">
      ...
      <div class="text-center">
        <n-pagination class="justify-content-center" v-model:page="page" v-model:page-size="pageSize" :item-count="currentProjectsCount"
