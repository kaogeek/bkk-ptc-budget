...
if (!process.env.DB_NAME || !process.env.TOKEN_SECRET) {
...
function generateAccessToken(payload: string | Buffer | object) {
...
app.get("/api/images/:filename", (req, res) => {
  ...
  if (existsSync(filePath)) {
    ...
    const fallbackFilePath = join(__dirname, "uploads/img", "og_image.png");
...
app.post("/api/update/file_name_id", async (request, response) => {
  ...
  data_list = data_list.filter(
    (item: any) => item.file_name_db !== file_name_db
  );
...
// add new community
app.post("/api/community/", async (request, response) => {
  const { community, districtId, subDistrictId } = request.body;

  try {
    const results = await pool.query(
      "INSERT INTO community (communityname, district_id, subdistrict_id) VALUES ($1, $2, $3) RETURNING *",
      [community, districtId, subDistrictId]
    );
...
app.use("/api/api-docs", serve, setup(specs, { explorer: true }));
