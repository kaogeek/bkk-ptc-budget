  ...
  export default {
    ...
    methods: {
      ...
      try {
          ...
          const params  = {}
          const json    = {
              "list_update" : {"title": this.title, "content": this.content, "fullname":this.fullname, "Staff_name":this.role, "user_id":this.user_id, "img_name":img_name},
              "row_id" : row_id
          }
          const options = {
          headers: {
      ...
      try {
          ...
          const params  = {}
          const json    = {"id" : id, "title" : this.title, "content" : this.content, "row_id" : row_id, "img_name":img_name}
          const options = {
          headers: {
              'Authorization': 'Bearer ' + this.api_token
          }
      ...
      async api_update_image_id() {
        ...
        try {
          const response = await axios.post(this.api_url + 'api/upload/image', formData, {
            headers: {
      ...
      delete_file(file_name){
        axios.delete(this.api_url + 'api/delete/chart_img/'+file_name)
          .then(response => {
            // pass
          })
          .catch(error => {
...
<template >
  <div class="container m-0 p-0" style="margin-top: 44px;">
      <div class="row">
        ...
        <div v-if="this.loading === true" class="border-grey bg-white p-3 tab-content" style="margin-top:64px;">
        ...
        <template v-if="this.loading === false">
          ...
          <div v-if="this.data_list.length != 0 && this.edit == false" v-for="item in this.data_list" class="col-md-10 p-2">
            <div class="p-2 ef" style="background-color: rgb(221, 221, 221); border-radius: 10px">
                <!-- <p>item : {{ item }}</p> -->
                <div class="text-center d-flex align-items-center justify-content-center">
                  <p class="fw-bold" style="margin-right: auto;">{{item.title}}</p>
                  <template v-if="item.user_id == this.user_id">
                    <a style="color: #535353" class="p-1" @click="bun_edit(item)"> <i class="bi bi-pencil-square"></i> </a>
                  </template>
                </div>
                ...
                <div class="d-flex justify-content-between">
                  <span v-if="item.Staff_name == 5" class="pt-3 text-start">อัพเดทโดย {{item.fullname}} (เจ้าหน้าที่ ศูนย์เด็กเล็ก)</span>
                  <span v-if="item.Staff_name == 4" class="pt-3 text-start">อัพเดทโดย {{item.fullname}} (เจ้าหน้าที่ กทม)</span>
                  <span v-if="item.Staff_name == 1" class="pt-3 text-start">อัพเดทโดย {{item.fullname}} (คณะกรรมการชุมชน)</span>
          ...
          <div v-if="this.edit == true" class="col-md-10 p-2 ef">
            <div class="p-2" style="background-color: rgb(221, 221, 221); border-radius: 10px">
              ...
              <div class=" pt-2 text-end">
                <button  type="button" class="btn btn-danger ef m-2" @click="button_edit_chat(0)">Cancel</button>
                <button v-if="this.title != '' && this.content != ''" type="button" class="btn btn-light ef m-2" @click="button_edit_chat(1)">Update</button>
              </div>
            </div>
          </div>
        </template>
