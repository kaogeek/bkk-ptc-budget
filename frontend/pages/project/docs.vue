    ...
    methods: {
      ...
      async api_edit_list_budget_id (id, row_id, list_name, budget) {
        try {
            ...
            const params  = {}
            const json    = {"id" : id, "list_name" : list_name, "budget" : budget,"row_id" : row_id}
            const options = {
              headers: {
                  'Authorization': 'Bearer ' + this.api_token
              }
      ...
      handleFormSubmit(){
          ...
          axios.post(this.api_url + 'api/upload/doc', formData, {
            headers: {
      ...
      delete_file(file_name){
        ...
        confirmButtonText: 'ตกลง',
          ...
          if (result.isConfirmed) {
            axios.delete(this.api_url + 'api/delete/doc/'+file_name)
            .then(response => {
      ...
      bun_edit_doc(i){
        if (i === 1){
          axios.delete(this.api_url+'api/delete/doc/'+this.file_name_db)
            .then(response => {
              setTimeout(() => { this.api_update_file_name_id (this.id, this.file_name_db) }, 500)
              setTimeout(() => { 
                this.api_get_project_id()
      ...
      bun_edit_table(i){
        ...
        if (i === 1 ){
          this.api_edit_list_budget_id(this.edit_item_id, this.id, this.list_name, this.budget)
          setTimeout(() => { 
              this.api_get_project_id()
              this.edit_table   = false
              this.edit_item    = false
  ...
  <div class="container m-0 p-0 pt-2">
      <div class="row ef">
        <div  class="col-md-10  ef">
          <span>ตารางแผนการใช้เงินสำหรับโครงการ</span> <span >{{ this.data_list.name }}</span>
          
          <!-- table -->
          <div>
            <!-- Edit -->
            <template v-if="this.edit_table == false && this.edit_item == false && this.data_list.create_email == this.create_email || this.role == 4">
              ...
              <table class="table">
                <thead>
                  <tr>
                    ...
                    <th v-if="this.edit_table == true" scope="col" class="text-end">แก้ไข</th>
                  </tr>
                </thead>
                <tbody>
                  ...
                  <template v-for="item in this.data_list.list_budget.data">
                    <tr class="small ef" style="font-size: 17px;">
                      <template v-if="item.id == this.edit_item_id">
                        ...
                        <th class="text-end fw-normal bg-warning">{{ item.budget }} บาท</th>
                      </template>
                      <template v-else>
                      ...
                      <th v-if="this.edit_table == true" class="text-center" style="width : 10px"> <i class="bi bi-pencil-square" @click="bun_edit_item(item)"></i> </th>
                    </tr>
                  </template>
                  ...
                  <!--ยอดรวม-->
                  <template v-if="this.edit_table == false && this.data_list.list_budget.data.length != 0">
                    <tr  class="small" style="font-size: 17px;">
                      <th>รวมทั้งสิ้น</th>
                  ...
                  <!--เพิ่มรายการ-->
                  <template v-if="this.edit_table == false && this.data_list.create_email == this.create_email || this.role == 4">
                    <tr class="small" style="font-size: 17px;">
                      <th style="width : 70%">
                        <input type="text"  class="form-control" placeholder="รายการ" v-model="list_name">
                      </th>
                      <th class="text-end" >
                        <div class="text-end">
                          <input type="number" class="form-control" placeholder="งบประมาณ" v-model="budget" min="0">
                        </div>
                      </th>
                    </tr>
                    ...
                    <tr class="small" style="font-size: 17px;">
                      ...
                      <th class="text-end" >
                        <div class="text-end">
                          <button v-if="this.list_name != '' && this.budget != '' && this.edit_item == false" style="width : 100px" type="button" class="btn btn-primary ef m-2" @click="bun_add_list()">เพิ่ม</button>
                        </div>
                      </th>
                    </tr>
                  </template>
          ...
          <template v-if="this.edit_item == true">
            <div class=" pt-2 text-end">
              <button type="button" class="btn btn-danger ef m-2" @click="bun_edit_table(0)">ยกเลิก</button>
              <button v-if="this.list_name != '' && this.budget != ''" type="button" class="btn btn-primary ef m-2" @click="bun_edit_table(1)">แก้ไข</button>
            </div>
          </template>
          ...
          <template v-if="this.edit == false" v-for="item in this.data_list.file_doc.data">
            <div class="row mb-3 d-flex ef">
              ...
              <div class="col-9">
                <div class="row">
                  <div v-if="item.email == this.create_email && this.role != 4" class="col-12 fw-bold">{{ item.file_name }} <i class="bi bi-pencil-square" @click="bun_edit(item.file_name, item.file_name_db)"></i> </div>
                  <div v-else class="col-12 fw-bold">{{ item.file_name }} </div>
                  <div class="col-12">
                    <button v-if="item.email == this.create_email || this.role == 4" style="color : black" type="button" class="btn btn-link m-0 p-0" @click="download_file(item.file_name_db)">Download</button>
          ...
          <button v-if="this.showInput == false && this.data_list.file_doc.data.length <= 4 && this.role == 1 || this.role == 5 " type="button" class="btn btn-primary" @click="showFileInput">Add</button>
          <!-- form -->
              ...
              <div class="row">
                <div class="text-center">
                  ...
                  <template v-if="this.edit == false">
                    <button type="submit" class="btn btn-success btn-lg mb-1 pull-right m-1" style="background:#EF4D4E; border-radius: 4px;color:#FFFFFF;border-color:#EF4D4E;font-size:14px;">อัพโหลด</button>
                  </template>

                  <template v-else>
                    <button  type="button" class="btn btn-success btn-lg mb-1 pull-right m-1" style="background:#EF4D4E; border-radius: 4px;color:#FFFFFF;border-color:#EF4D4E;font-size:14px;" @click="bun_edit_doc(0)">ยกเลิก</button>
                    <button v-if="this.file_name != '' && this.selectedFile != null" type="button" class="btn btn-success btn-lg mb-1 pull-right m-1" style="background:#EF4D4E; border-radius: 4px;color:#FFFFFF;border-color:#EF4D4E;font-size:14px;" @click="bun_edit_doc(1)">แก้ไข</button>
                  </template>
