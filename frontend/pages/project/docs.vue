...
export default {
	...
	methods: {
		...
		async api_update_file_doc_id(row_id, file_name_db) {
			try {
				const url = baseUrl + 'api/update/doc_id'
				const params = {}
				const json = {
		...
		async api_update_file_name_id(row_id, file_name_db) {
			try {
				const url = baseUrl + 'api/update/file_name_id'
				const params = {}
				const json = {
		...
		handleFormSubmit() {
			...
			axios.post(baseUrl + 'api/upload/doc', formData, {
				headers: {
		...
		download_file(file_name) {
			const url = baseUrl + 'api/download/doc/' + file_name
		...
		delete_file(file_name) {
			Swal.fire({
				...
				if (result.isConfirmed) {
					axios.delete(baseUrl + 'api/delete/doc/' + file_name)
						.then(response => {
	...
	<div class="container m-0 p-0 pt-2">
		<div class="row ef">
			<div class="col-md-10  ef">
						...
						<div class="col-9">
							<div class="row">
								...
								<div class="col-12">
									...
									<button v-if="item.email == this.create_email" style="color : black" type="button"
										class="btn btn-link m-0 p-0" @click="download_file(item.file_name_db)">Download</button>
									<button v-if="item.email == this.create_email && this.role != 4" style="color : black" type="button"
										class="btn btn-link m-0 ps-2" @click="delete_file(item.file_name_db)">Delete</button>
								</div>
							</div>
						</div>
					</div>
				...
				<button
					v-if="this.showInput == false && this.data_list.file_doc.data.length <= 4 && this.role == 1 || this.role == 4"
					type="button" class="btn btn-primary" @click="showFileInput">Add</button>
				<!-- form -->
				<form v-if="this.showInput == true" action="#" @submit.prevent="handleFormSubmit">
					<div class="row mb-3">
						<span class="m-1">ชื่อเอกสาร</span>
						<div class="col-sm-10 pt-1">
							<input type="text" class="form-control" id="projectname" placeholder="ชื่อเอกสาร" required
								v-model="file_name">
						</div>
					</div>
					...
					<div class="row">
						<div class="text-center">
							<button type="submit" class="btn btn-success btn-lg mb-1 pull-right"
								style="background:#EF4D4E;;border-radius: 4px;color:#FFFFFF;border-color:#EF4D4E;font-size:14px;">อัพโหลด</button>
						</div>
					</div>
				</form>
