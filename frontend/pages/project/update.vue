...
export default {
	...
	methods: {
		...
		async api_update_chat_id(row_id) {
			try {
				const url = baseUrl + 'api/update/chat_id'
				const params = {}
				const json = {
			...
			<div v-if="this.role == 4 || this.role == 1" class="col-md-10 p-2">
				<div class="p-2" style="background-color: rgb(221, 221, 221); border-radius: 10px">
					...
					<div class="form-floating pt-2">
						<textarea style="height :150px" class="form-control" placeholder="Leave a comment here" id="floatingTextarea"
							v-model="content"></textarea>
						<label for="floatingTextarea">เนื้อหา</label>
					</div>
					<div class=" pt-2 text-end">
						<button v-if="this.title != '' && this.content != ''" type="button" class="btn btn-light ef"
							@click="button_updater()">Update</button>
					</div>
				</div>
			</div>
